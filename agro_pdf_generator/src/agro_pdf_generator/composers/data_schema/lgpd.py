...
def build_lgpd_consent(applicant: ApplicantData) -> LgpdConsentData:
    return LgpdConsentData(
        ...
        signature_cpf=applicant.cpf if len(applicant.cpf.replace(".", "").replace("-", "").replace(" ", "")) == 11 else "",
        signature_cnpj=applicant.cpf if len(applicant.cpf.replace(".", "").replace("-", "").replace(" ", "")) == 14 else ""
    )
