...
class WebhooksPaginator(CursorPaginator[Webhook]):
    """Cursor-based paginator for ``GET /api/v2/webhooks``.

    The Webhooks API does not use ``.json`` suffixes or offset ``page`` /
    ``per_page`` params. It uses ``page[size]`` / ``page[after]`` and returns
    ``links`` + ``meta`` like search export.
    """
    ...
    def __init__(
        ...
        super().__init__(http_client, "webhooks", params=params, per_page=page_size, limit=limit)
        self._next_url: Optional[str] = None

    def _extract_items(self, response: Dict[str, Any]) -> List[Webhook]:
    ...
    def _get_page_params(self) -> Dict[str, Any]:
        params: Dict[str, Any] = {"page[size]": self.per_page}
        if self._next_cursor and self._has_started:
            params["page[after]"] = self._next_cursor
        return params
...
class ZendeskPaginator:
    ...
    @staticmethod
    def create_ticket_audits_paginator(
        http_client: Any,
        ticket_id: int,
        *,
        ...
        per_page: int = 100,
        limit: Optional[int] = None,
    ) -> OffsetPaginator[Audit]:
        """Create paginator for ticket audits endpoint."""

        class TicketAuditsPaginator(OffsetPaginator[Audit]):
            def __init__(
                self,
                http_client: Any,
                path: str,
                ...
                super().__init__(http_client, path, params=params, per_page=per_page, limit=limit)
                self._filter_events = filter_events or []

            async def _fetch_page(self, page_params: Dict[str, Any]) -> Dict[str, Any]:
                query_params: List[Tuple[str, Any]] = list(page_params.items())
                for event_type in self._filter_events:
                    query_params.append(("filter_events[]", event_type))
                return await self.http_client.get(self.path, params=query_params)

            def _extract_items(self, response: Dict[str, Any]) -> List[Audit]:
                return [Audit(**a) for a in response.get("audits", [])]

        return TicketAuditsPaginator(
            http_client,
    ...
    @staticmethod
    def create_ticket_forms_paginator(
        http_client: Any, per_page: int = 100, limit: Optional[int] = None
    ) -> OffsetPaginator[TicketForm]:
        """Create paginator for ticket forms endpoint."""

        class TicketFormsPaginator(OffsetPaginator[TicketForm]):
            def _extract_items(self, response: Dict[str, Any]) -> List[TicketForm]:
                return [TicketForm(**f) for f in response.get("ticket_forms", [])]

        return TicketFormsPaginator(http_client, "ticket_forms.json", per_page=per_page, limit=limit)

    @staticmethod
    def create_custom_statuses_paginator(
        http_client: Any, per_page: int = 100, limit: Optional[int] = None
    ) -> OffsetPaginator[CustomStatus]:
        """Create paginator for custom statuses endpoint."""

        class CustomStatusesPaginator(OffsetPaginator[CustomStatus]):
            def _extract_items(self, response: Dict[str, Any]) -> List[CustomStatus]:
                return [CustomStatus(**s) for s in response.get("custom_statuses", [])]

        return CustomStatusesPaginator(http_client, "custom_statuses.json", per_page=per_page, limit=limit)

    @staticmethod
    def create_triggers_paginator(
        http_client: Any, per_page: int = 100, limit: Optional[int] = None
    ) -> OffsetPaginator[Trigger]:
        """Create paginator for triggers endpoint."""

        class TriggersPaginator(OffsetPaginator[Trigger]):
            def _extract_items(self, response: Dict[str, Any]) -> List[Trigger]:
                return [Trigger(**t) for t in response.get("triggers", [])]

        return TriggersPaginator(http_client, "triggers.json", per_page=per_page, limit=limit)

    @staticmethod
    def create_automations_paginator(
        http_client: Any, per_page: int = 100, limit: Optional[int] = None
    ) -> OffsetPaginator[Automation]:
        """Create paginator for automations endpoint."""

        class AutomationsPaginator(OffsetPaginator[Automation]):
            def _extract_items(self, response: Dict[str, Any]) -> List[Automation]:
                return [Automation(**a) for a in response.get("automations", [])]

        return AutomationsPaginator(http_client, "automations.json", per_page=per_page, limit=limit)

    @staticmethod
    def create_webhooks_paginator(
        http_client: Any, per_page: int = 100, limit: Optional[int] = None
    ) -> WebhooksPaginator:
        """Create cursor paginator for ``GET /api/v2/webhooks`` (no ``.json`` suffix)."""
        return WebhooksPaginator(http_client, page_size=per_page, limit=limit)

    @staticmethod
    def create_sla_policies_paginator(
        http_client: Any, per_page: int = 100, limit: Optional[int] = None
    ) -> OffsetPaginator[SlaPolicy]:
        """Create paginator for SLA policies endpoint."""

        class SlaPoliciesPaginator(OffsetPaginator[SlaPolicy]):
            def _extract_items(self, response: Dict[str, Any]) -> List[SlaPolicy]:
                return [SlaPolicy(**p) for p in response.get("sla_policies", [])]

        return SlaPoliciesPaginator(http_client, "slas/policies.json", per_page=per_page, limit=limit)

    @staticmethod
    def create_brands_paginator(
        http_client: Any, per_page: int = 100, limit: Optional[int] = None
    ) -> OffsetPaginator[Brand]:
        """Create paginator for brands endpoint."""

        class BrandsPaginator(OffsetPaginator[Brand]):
            def _extract_items(self, response: Dict[str, Any]) -> List[Brand]:
                return [Brand(**b) for b in response.get("brands", [])]

        return BrandsPaginator(http_client, "brands.json", per_page=per_page, limit=limit)
