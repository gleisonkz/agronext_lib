...
class AttachmentsClient:
    ...
    async def upload(
        ...
        # Mirror HTTPClient._create_client: BasicAuth for token mode, Bearer header for OAuth.
        # auth_tuple is None in OAuth mode, so indexing it directly would crash here.
        auth: Optional[httpx.BasicAuth] = None
        if self._config.auth_tuple:
            auth = httpx.BasicAuth(
                username=self._config.auth_tuple[0],
                password=self._config.auth_tuple[1],
            )
        elif self._config.oauth_token:
            headers["Authorization"] = f"Bearer {self._config.oauth_token}"
