...
from ..exceptions import ZendeskValidationException
from ..models import Audit, Comment, EnrichedTicket, JobStatus, Organization, RequesterInput, Ticket, TicketField, User
...
class CommentsClient(BaseClient):
    ...
    async def redact(self, ticket_id: int, comment_id: int, text: str) -> Comment:
        """Permanently redact plain text from a comment (legacy endpoint).

        Uses ``PUT tickets/{id}/comments/{id}/redact.json`` with a ``text`` payload.
        For Agent Workspace HTML redaction or attachment URL redaction, use
        :meth:`redact_html` or :meth:`redact_attachments` instead.
    ...
    async def redact_html(self, ticket_id: int, comment_id: int, html_body: str) -> Comment:
        """Redact HTML content from a comment (Agent Workspace endpoint).

        Uses ``PUT comment_redactions/{comment_id}.json`` with ``html_body`` containing
        ``<redact>...</redact>`` markers. Distinct from :meth:`redact`, which uses the
        legacy plain-text redaction endpoint.

        Args:
            ticket_id: The ticket's ID
            comment_id: The comment's ID
            html_body: HTML body with redaction markers

        Returns:
            Updated Comment object
        """
        response = await self._put(
            f"comment_redactions/{comment_id}.json",
            json={"ticket_id": ticket_id, "html_body": html_body},
        )
        return Comment(**response["comment"])

    async def redact_attachments(
        self,
        ticket_id: int,
        comment_id: int,
        external_attachment_urls: List[str],
    ) -> Comment:
        """Redact attachments from a comment by content URL.

        Uses ``PUT comment_redactions/{comment_id}.json`` with
        ``external_attachment_urls``. Distinct from :meth:`redact`, which redacts
        plain text via the legacy comments redact endpoint.

        Args:
            ticket_id: The ticket's ID
            comment_id: The comment's ID
            external_attachment_urls: Attachment content URLs to redact
        ...
        response = await self._put(
            f"comment_redactions/{comment_id}.json",
            json={"ticket_id": ticket_id, "external_attachment_urls": external_attachment_urls},
        )
        return Comment(**response["comment"])


class AuditsClient(BaseClient):
    """Client for Ticket Audits API."""
    ...
    def list(
        ...
        *,
        ...
        """Get paginated audits for a ticket.

        Args:
            ticket_id: The ticket's ID
            filter_events: Optional event types to pass as ``filter_events[]`` query params
            per_page: Number of audits per page (max 100)
            limit: Maximum number of items to return when iterating
        """
        return ZendeskPaginator.create_ticket_audits_paginator(
            self._http,
            ticket_id,
    ...
    @staticmethod
    def filter_events_by_type(audits: List[Audit], event_types: List[str]) -> List[Audit]:
        """Client-side helper to keep only audits containing matching event types."""
        ...
        for audit in audits:
            ...
            matching = [
                event
                for event in events
                if (event.type in allowed) or (event.field_name in allowed)
            ]
            if matching:
                filtered.append(audit.model_copy(update={"events": matching}))
        return filtered
...
class TicketsClient(BaseClient):
    ...
    def audits(self) -> AuditsClient:
        """Access ticket audits API."""
        return AuditsClient(self._http)

    @staticmethod
    def _validate_requester_inputs(
        requester: Optional[Union[RequesterInput, Dict[str, Any]]],
        requester_id: Optional[int],
    ) -> None:
        if requester is not None and requester_id is not None:
            raise ZendeskValidationException(
                "Cannot pass both requester and requester_id",
                field="requester",
            )
    ...
    async def create(
        ...
        *,
        ...
        requester: Optional[Union[RequesterInput, Dict[str, Any]]] = None,
        ...
        Args:
            ...
            requester: Inline requester dict with name and/or email (mutually exclusive with requester_id)
            uploads: List of upload tokens from attachments.upload() to attach files
            brand_id: Brand ID for the ticket (Enterprise)
            ticket_form_id: Ticket form ID to associate with the ticket
            custom_status_id: Custom ticket status ID (Enterprise)
        ...
        """
        self._validate_requester_inputs(requester, requester_id)
        ...
        if requester is not None:
            ticket_data["requester"] = dict(requester)
        if requester_id is not None:
            ticket_data["requester_id"] = requester_id
        if submitter_id is not None:
            ticket_data["submitter_id"] = submitter_id
        if brand_id is not None:
            ticket_data["brand_id"] = brand_id
        if ticket_form_id is not None:
            ticket_data["ticket_form_id"] = ticket_form_id
        if custom_status_id is not None:
            ticket_data["custom_status_id"] = custom_status_id
    ...
    async def update(
        ...
        *,
        ...
        Args:
            ...
            brand_id: Change brand (Enterprise)
            ticket_form_id: Change ticket form
            custom_status_id: Change custom ticket status (Enterprise)
    ...
    async def update_many(self, ticket_ids: List[int], **fields: Any) -> JobStatus:
        """Batch-update multiple tickets with shared field values.

        Submits ``PUT tickets/update_many.json?ids=...`` and returns the job status.
        Poll completion via ``client.job_statuses.wait_until_done(job.id)``.
        """
        if not ticket_ids:
            raise ZendeskValidationException("ticket_ids must not be empty", field="ticket_ids")

        unique_ids = list(dict.fromkeys(ticket_ids))[:100]
        ids_param = ",".join(str(tid) for tid in unique_ids)
        response = await self._put(
            f"tickets/update_many.json?ids={ids_param}",
            json={"ticket": fields},
        )
